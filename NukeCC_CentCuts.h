...
int  GetTargetPlane( int targetID=1);
 ...
 class PassMuCurveCut: public Cut<UNIVERSE, EVENT>
 {
	...
	private:
		...
		{
           return univ.GetMuonCurve() <=0;  //nu 
//       	   return univ.GetMuonCurve() >=0;  //anu 
		}
 };
 ...
 class IsDIS: public Cut<UNIVERSE, EVENT>
 {
    ...
    private:
      ...
      bool checkCut(const UNIVERSE& univ,  EVENT& /*evt*/) const override
      {
        ...
        return  univ.GetQ2RecoGeV() >= m_Q2Min && univ.GetWRecoGeV() >= m_WMin; //make change in q2 < to >
      }
//    private:
...
const double MIN_PROB;
  ...
  class InHexagon: public Cut<UNIVERSE, EVENT>
     ...
     bool checkCut(const UNIVERSE& univ, EVENT& /*evt*/) const override
   {
    ...
    double x = univ.GetVecElem("NukeCC_vtx",0);
    double y = univ.GetVecElem("NukeCC_vtx",1);

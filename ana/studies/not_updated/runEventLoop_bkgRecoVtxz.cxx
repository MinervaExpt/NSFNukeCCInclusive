              ...
              for (auto v : variables2d){
                ...
                if(cutter->PassTruth(universe,helicity) ){ // CC, antineutrino, fiducial cuts
                  if (cutter->PassTrueMuEnergyCut(universe) && cutter->PassTrueThetaCut(universe)){
                    ...
                    if(cutter->IsInTrueMaterial(universe,targetID, targetZ) && universe->GetInt("truth_targetID") == targetID){
                      v->m_selected_mc_reco_Sg.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());
                    }
                    // material + other target bkg
                    ...
                    // TRUE INFORMATION
                      ...
                      if(universe->GetInt("truth_targetID") == targetID){
                        if(cutter->IsInTrueMaterial(universe,targetID, 26)){
                          v->m_selected_mc_reco_Fe.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight()); 
                        }
                        else if(cutter->IsInTrueMaterial(universe,targetID, 82) ){
                          v->m_selected_mc_reco_Pb.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight()); 
                        }
                        else if(cutter->IsInTrueMaterial(universe,targetID, 6)){
                          v->m_selected_mc_reco_C.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight()); 
                      ...
                      // bkg from outside target3
                        ...
                        if(universe->GetInt("truth_targetZ") == 26){
                          v->m_selected_mc_reco_otherFe.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight()); 
                        }
                        else if(universe->GetInt("truth_targetZ") == 82){
                          v->m_selected_mc_reco_otherPb.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight()); 
                        }
                        else if(universe->GetInt("truth_targetZ") == 12){
                          v->m_selected_mc_reco_otherC.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight()); 
                        }
                        else{
                          // all the other bkg (plastic)
                          v->m_selected_mc_reco_Plastic.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight()); 
                          if (universe->GetInt("truth_targetZ") != 0){
                            std::cout << "Target Z = " << universe->GetInt("truth_targetZ")<< std::endl;
                          }
                        }
                ...
                if(cutter->PassTruth(universe,helicity) ){ // CC, antineutrino, fiducial cuts
                  if (cutter->PassTrueMuEnergyCut(universe) && cutter->PassTrueThetaCut(universe)){
                    ...
                    // material + other target bkg
                      ...
                      v->m_selected_mc_sb.GetComponentHist("MatBkg")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                      //materialbkg++;
                      // bkg in t3
                      if(universe->GetInt("truth_targetID") == targetID){
                        if(cutter->IsInTrueMaterial(universe,targetID, 26)){
                          v->m_selected_mc_sb.GetComponentHist("FeT3")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                          //fe++ ;
                        }
                        else if(cutter->IsInTrueMaterial(universe,targetID, 82) ){
                          v->m_selected_mc_sb.GetComponentHist("PbT3")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                          //pb++;
                        }
                        else if(cutter->IsInTrueMaterial(universe,targetID, 6)){
                          v->m_selected_mc_sb.GetComponentHist("CT3")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                          //carbon++;
                        }
                        else{
                          //ch++;
                      ...
                      // bkg from outside target3
                        ...
                        if(universe->GetInt("truth_targetZ") == 26){
                          v->m_selected_mc_sb.GetComponentHist("otherFe")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                          //otherTargetFe++;
                        }
                        else if(universe->GetInt("truth_targetZ") == 82){
                        v->m_selected_mc_sb.GetComponentHist("otherPb")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                          //otherTargetPb++;
                        }
                        else if(universe->GetInt("truth_targetZ") == 12){
                          v->m_selected_mc_sb.GetComponentHist("otherC")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                          //otherTargetC++;
                        }
                        else{
                          // water target!!
                          if(517 < universe->GetVertexZTrueMy() &&  universe->GetVertexZTrueMy() < 544){
                            v->m_selected_mc_reco_bkg_water.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                          }
                          else{
                            //otherTargetPlastic++;
                            v->m_selected_mc_sb.GetComponentHist("Plastic")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());

                            // different sources of plastic!
                            if (universe->GetInt("mc_targetNucleus") == 2212){ // H
                              v->m_selected_mc_plastic.GetComponentHist("H")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            };
                            if (universe->GetInt("mc_targetNucleus") == 1000060120){ // C
                              v->m_selected_mc_plastic.GetComponentHist("C")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            };
                            if (universe->GetInt("mc_targetNucleus") == 1000080160){ //O
                              v->m_selected_mc_plastic.GetComponentHist("O")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            };
                            if (universe->GetInt("mc_targetNucleus") == 1000130270){ // Al
                              v->m_selected_mc_plastic.GetComponentHist("Al")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            };
                            if (universe->GetInt("mc_targetNucleus") == 1000140280){ // Si
                              v->m_selected_mc_plastic.GetComponentHist("Si")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            };
                            if (universe->GetInt("mc_targetNucleus") == 1000220480){ // Ti
                              v->m_selected_mc_plastic.GetComponentHist("Ti")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            };
                            if (universe->GetInt("mc_targetNucleus") == 1000070140){ // N
                              v->m_selected_mc_plastic.GetComponentHist("N")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            };
                            if (universe->GetInt("mc_targetNucleus") == 1000020040){ // He
                              v->m_selected_mc_plastic.GetComponentHist("He")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            };
                          }
                        }
                      } // end of bkg from outside t3

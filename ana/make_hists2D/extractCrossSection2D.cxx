...
------"<<std::endl;
  ...
  TFile *fUnfold;
  if (trackerFlux == true){
    fUnfold = new TFile( Form("%s/CrossSection2D_t%d_z%02d_%s.root", outdir.c_str(), targetID, targetZ, plist_string.c_str() ), "recreate" );
  }
  else{
    fUnfold = new TFile( Form("%s/CrossSection2D_Daisy_t%d_z%02d_%s.root", outdir.c_str(), targetID, targetZ, plist_string.c_str() ), "recreate" );
  }
  ...
  for(int t = 0; t < targetIDs.size(); t++){
    ...
    if(RunCodeWithSystematics){
          eventLoop = Form("%s/BackgroundSubtracted/BkgSubtracted_EventSelection2D_%s_t%d_z%02d_sys.root", outdir.c_str(), plist_string.c_str(), targetIDs[t], targetZ);
          migr = Form("%s/Migration/Migration2D_%s_t%d_z%02d_sys.root", outdir.c_str(), plist_string.c_str(), targetIDs[t], targetZ);
    ...
    for(const auto&var: vars){
      ...
      auto migration  = dynamic_cast<MnvH2D*> (fMigration->Get( Form("response2d_%s_migration", var.c_str())));
      auto migration_reco  = dynamic_cast<MnvH2D*> (fMigration->Get( Form("response2d_%s_reco", var.c_str())));
      auto migration_truth  = dynamic_cast<MnvH2D*> (fMigration->Get( Form("response2d_%s_truth", var.c_str())));
      auto effNum = dynamic_cast<MnvH2D*>(fEfficiency->Get( Form("h_mc_%s", var.c_str())));
      auto effDenom = dynamic_cast<MnvH2D*>(fEfficiency->Get( Form("h_truth_%s", var.c_str())));
      auto effDenom_QE = dynamic_cast<MnvH2D*>(fEfficiency->Get( Form("h_truth_QE_%s", var.c_str())));
      auto effDenom_RES = dynamic_cast<MnvH2D*>(fEfficiency->Get( Form("h_truth_RES_%s", var.c_str())));
      auto effDenom_DIS = dynamic_cast<MnvH2D*>(fEfficiency->Get( Form("h_truth_DIS_%s", var.c_str())));
      auto effDenom_Other = dynamic_cast<MnvH2D*>(fEfficiency->Get( Form("h_truth_Other_%s", var.c_str())));
      auto effDenom_2p2h = dynamic_cast<MnvH2D*>(fEfficiency->Get( Form("h_truth_2p2h_%s", var.c_str())));
      ...
      for(const auto& prefix: prefixes){
        ...
        auto bkgSubtracted = dynamic_cast<MnvH2D*>(fEventLoop->Get( Form("h_bkg_subtracted_%s_%s", prefix.c_str(), var.c_str())));
 ...
 // CROSS-SECTION EXTRACTION
  ...
  for(const auto&var: vars){
    ...
    auto effNum_combined = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("efficiency_numerator_%d_%s", targetIDs[0],var.c_str() )));
    auto simEventRate_combined = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("simEventRate_%d_%s", targetIDs[0],var.c_str() )));
    auto simEventRate_QE_combined = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("simEventRate_QE_%d_%s", targetIDs[0],var.c_str() )));
    auto simEventRate_RES_combined = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("simEventRate_RES_%d_%s", targetIDs[0],var.c_str() )));
    auto simEventRate_DIS_combined = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("simEventRate_DIS_%d_%s", targetIDs[0],var.c_str() )));
    auto simEventRate_Other_combined = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("simEventRate_Other_%d_%s", targetIDs[0],var.c_str() )));
    auto simEventRate_2p2h_combined = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("simEventRate_2p2h_%d_%s", targetIDs[0],var.c_str() )));
      
    for(int t = 1; t < targetIDs.size(); t++){
      auto effNum = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("efficiency_numerator_%d_%s", targetIDs[t],var.c_str() )));
      auto simEventRate = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("simEventRate_%d_%s", targetIDs[t],var.c_str() )));
      auto simEventRate_QE = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("simEventRate_QE_%d_%s", targetIDs[t],var.c_str() )));
      auto simEventRate_RES = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("simEventRate_RES_%d_%s", targetIDs[t],var.c_str() )));
      auto simEventRate_DIS = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("simEventRate_DIS_%d_%s", targetIDs[t],var.c_str() )));
      auto simEventRate_Other = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("simEventRate_Other_%d_%s", targetIDs[t],var.c_str() )));
      auto simEventRate_2p2h = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("simEventRate_2p2h_%d_%s", targetIDs[t],var.c_str() )));
      ...
      fUnfold->Delete(( Form("efficiency_numerator_%d_%s", targetIDs[t],var.c_str() )));
    ...
    for(const auto& prefix: prefixes){
      ...
      auto unfolded_combined = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("unfolded_effCorrected_%d_%s_%s", targetIDs[0], prefix.c_str(),var.c_str() )));
      
      for(int t = 1; t < targetIDs.size(); t++){
        auto unfolded = dynamic_cast<MnvH2D*>(fUnfold->Get( Form("unfolded_effCorrected_%d_%s_%s", targetIDs[t], prefix.c_str(),var.c_str() )));

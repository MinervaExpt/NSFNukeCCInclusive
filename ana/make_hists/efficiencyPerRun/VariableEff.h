...
class Variable : public PlotUtils::VariableBase<NUKECC_ANA::CVUniverse> {
 ...
 public:
  ...
  HW m_selected_mc_reco_true;
  HW m_selected_mc_MLreco;
  ...
  void InitializeAllHistograms(T univs) {
    ...
    MH1D* dummy_selected_mc_MLreco = new MH1D(Form("selected_mc_MLreco_%s", name), name,
                                      GetNBins(), bins.data());
    m_selected_mc_MLreco = HW(dummy_selected_mc_MLreco, univs, clear_bands);

    MH1D* dummy_selected_mc_reco_true = new MH1D(Form("selected_mc_reco_%s", name), name,
                                      GetNBins(), bins.data());
    m_selected_mc_reco_true = HW(dummy_selected_mc_reco_true, univs, clear_bands);
 ...
 // PlotUtils::MnvH1D* data = new PlotUtils::MnvH1D(
  ...
  //  m_selected_data_sb = PlotUtils::HistFolio<PlotUtils::MnvH1D>(
    ...
    m_selected_mc_sb.AddComponentHist("Truth");
    m_selected_mc_sb.AddComponentHist("MC");
    delete dummy_selected_mc_reco;
    delete dummy_selected_mc_MLreco;
 ...
 void WriteAllHistogramsToFile(TFile& f, bool isMC) const {
    ...
    // selected mc reco
    if(isMC)  {
      m_selected_mc_MLreco.hist->Write();

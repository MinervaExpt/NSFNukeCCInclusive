...
def getspline(material):
    ...
    tf = ROOT.TFile("$MPARAMFILESROOT/data/GENIE/spline_files/gxspl-nuclear-MINERVA_Full_v2126.root")
    splinename = None
    if material == "carbon":
        splinename = "nu_mu_bar_C12/tot_cc" # Anezka change to numu_bar
    if material == "lead":
        splinename = "nu_mu_bar_Pb207/tot_cc" # Anezka change to numu_bar
    if material == "iron":
        splinename = "nu_mu_bar_Fe56/tot_cc" # Anezka change to numu_bar
    if material == "oxygen":
        splinename = "nu_mu_bar_O16/tot_cc" # Anezka change to numu_bar
...
def getntargets(material, isMC, NNucleons = True):
    ...
    if material == "tracker":
        return trackerfunc(5890.0, 8467.0, isMC, apothem), usual_uncertainty # Jeffrey's  original cut
        #return trackerfunc(5980.0, 8422.0, isMC, apothem), usual_uncertainty # MY CUT
...
def safe_getntargets(ntargets_material):
    if ntargets_material == "tracker":
        PlotUtils.TargetUtils.Get().SetDistToDivCut(0)
        out = PlotUtils.TargetUtils.Get().GetTrackerNAtoms(5890.0, 8467.0, True, 850.0) # old Jeffrey
        #out = PlotUtils.TargetUtils.Get().GetTrackerNAtoms(5980.0, 8422.0, True, 850.0) # new Anezka
        ...
        out = PlotUtils.TargetUtils.Get().GetTrackerNCarbonAtoms(5890.0, 8467.0, True, 850.0) # old Jeffrey
        #out = PlotUtils.TargetUtils.Get().GetTrackerNCarbonAtoms(5980.0, 8422.0, True, 850.0)
        return out, 0.005

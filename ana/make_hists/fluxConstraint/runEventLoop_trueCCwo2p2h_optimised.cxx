...
int main(int argc, char *argv[]){
  ...
  // Tuples ?
  const std::string mc_file_list(Form("../../include/playlists/MasterAnaDev_MC_%s_MuonKludged.txt", plist_string.c_str()));
    ...
    if(i%500000==0) std::cout << (i/1000) << "k " << std::endl;
      ...
      if(isMC){     
        for (auto band_GROUP : error_bands_GROUP){
          ...
          // TARGET 1 Fe
          if(cutter->IsInTrueMaterial(band_GROUP.front(), 1, 26, /*anyTrakerMod*/false)){
            t1fe++;
            for (auto universe : band_GROUP){
          ...
          // TARGET 1 Pb
          if(cutter->IsInTrueMaterial(band_GROUP.front(), 1, 82, /*anyTrakerMod*/false)){
            t1pb++;
            for (auto universe : band_GROUP){
              for (auto v : variables) {
          ...
          // TARGET 2 Fe
          if(cutter->IsInTrueMaterial(band_GROUP.front(), 2, 26, /*anyTrakerMod*/false)){
            t2fe++;
            for (auto universe : band_GROUP){
              for (auto v : variables) {
          ...
          // TARGET 2 Pb
          if(cutter->IsInTrueMaterial(band_GROUP.front(), 2, 82, /*anyTrakerMod*/false)){
            t2pb++;
            for (auto universe : band_GROUP){
              for (auto v : variables) {
          ...
          // TARGET 3 Fe
          if(cutter->IsInTrueMaterial(band_GROUP.front(), 3, 26, /*anyTrakerMod*/false)){
            t3fe++;
            for (auto universe : band_GROUP){
              for (auto v : variables){
          ...
          // TARGET 3 Pb
          if(cutter->IsInTrueMaterial(band_GROUP.front(), 3, 82, /*anyTrakerMod*/false)){
            t3pb++;
            for (auto universe : band_GROUP){
              for (auto v : variables) {
          ...
          // TARGET 3 C
          if(cutter->IsInTrueMaterial(band_GROUP.front(), 3, 6, /*anyTrakerMod*/false)){
            t3c++;
            for (auto universe : band_GROUP){
          ...
          // TARGET 4 Pb
          if(cutter->IsInTrueMaterial(band_GROUP.front(), 4, 82, /*anyTrakerMod*/false)){
            t4pb++;
            for (auto universe : band_GROUP){
              for (auto v : variables) {
          ...
          // TARGET 5 Fe
          if(cutter->IsInTrueMaterial(band_GROUP.front(), 5, 26, /*anyTrakerMod*/false)){
            t5fe++;
            for (auto universe : band_GROUP){
              for (auto v : variables) {
          ...
          // TARGET 5 Pb
          if(cutter->IsInTrueMaterial(band_GROUP.front(), 5, 82, /*anyTrakerMod*/false)){
            t5pb++;
            for (auto universe : band_GROUP){
              for (auto v : variables) {

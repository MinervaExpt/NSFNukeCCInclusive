...
------"<<std::endl;
  ...
  const string playlist= argv[4];
   ...
   if(RunCodeWithSystematics){
       histFileName = Form("%s/Hists_PlasticBackgd_with_SYS_t%d_z%02d_AntiNu_%s_.root", outdir.c_str(), targetID, targetZ, getenv("NUKECC_TAG") ); 
     }
   else{
       histFileName = Form("%s/Hists_PlasticBackgd_without_SYS_t%d_z%02d_AntiNu_%s_.root", outdir.c_str(), targetID, targetZ, getenv("NUKECC_TAG"));
     } 
 ...
 if(var == "planeDNN"){
  ...
  if( ! tune_fiducial ){
    scaleFactor = new TFile( Form("%s/CHScaleFactors_t%d_z%02d_%s_%s.root", outdir.c_str(), targetID, targetZ, playlist.c_str(), getenv("NUKECC_TAG") ), "recreate" );
    assert(scaleFactor);
  }
...
void  CalcScaleFactorAndTuneBG( MnvPlotter mnvPlotter, vector<MnvH1D*> histos_fit, vector<MnvH1D*> histos_mc_tuning, const string var, int targetID, int targetZ, double dataMCScale, TFile *scaleFactor, bool fixUS, bool fixDS, string playlist, bool makeDISPlots, const string outdir){
  ...
  MnvH1D* untuned_mc_sideband_regDS      = (MnvH1D*)histos_mc_tuning[3]->Clone("untuned_mc_ds");

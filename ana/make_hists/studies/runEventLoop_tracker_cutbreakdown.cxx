...
int main(int argc, char *argv[]){
  ...
  if(RunCodeWithSystematics){
    histFileName += Form("/EventSelection_CutBreakdown_%s_t%d_z%02d_sys.root", plist_string.c_str(), targetID, targetZ);
  }

  else{
    histFileName += Form("/EventSelection_CutBreakdown_%s_t%d_z%02d_nosys.root", plist_string.c_str(), targetID, targetZ);
  } 
...
void FillVariable( PlotUtils::ChainWrapper* chain, HelicityType::t_HelicityType helicity, NukeCCUtilsNSF *utils , NukeCC_Cuts *cutter ,NukeCC_Binning  *binsDef ,std::vector<Var*>& variables,std::vector<Var2D*>& variables2d,bool isMC,int targetID, int targetZ, const string playlist, bool doDIS){
      ...
      if(isMC){     
        for (auto band : error_bands){
          ...
          for (auto universe : error_band_universes){
            ...
            if(! universe->GetInt((universe->GetAnaToolName() + "ANN_in_fiducial_area").c_str()) ) continue;
            reco1b++;

            if(!cutter->PassZDistCut(universe,helicity)) continue;
            reco1c++;

            if(!cutter->PassDistToDivisionCut(universe)) continue;
            reco1d++;
            
            if(universe->GetEnu() * mev_to_gev >  120. ) continue;
            reco1e++;
  ...
  std::cout << "Helicity cut = " << reco1a << std::endl;
  std::cout << "Fiducal cut = " << reco1b << std::endl;
  std::cout << "Z dist cut = " << reco1c << std::endl;
  std::cout << "Dist to div cut = " << reco1d << std::endl;
  std::cout << "Less than 120 GeV cut = " << reco1e << std::endl;
  std::cout << "Dead time cut = " << reco1f << std::endl;
  std::cout << "Mu curve cut = " << reco1g << std::endl;
  std::cout << "Mu coil cut = " << reco1h << std::endl;
  std::cout << "-999 cut = " << reco1i << std::endl;

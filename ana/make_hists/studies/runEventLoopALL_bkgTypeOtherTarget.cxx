...
#include "../../include/VariableRun_BkgType.h"  
...
void FillVariable( PlotUtils::ChainWrapper* chain, HelicityType::t_HelicityType helicity, NukeCCUtilsNSF *utils , NukeCC_Cuts *cutter ,NukeCC_Binning  *binsDef ,std::vector<Var*>& variables,std::vector<Var2D*>& variables2d,bool isMC,int targetID, int targetZ, const string playlist, bool doDIS){
  ...
  //for(int t = 0; t < targetIDs.size(); t++){
    ...
    for(int i=0; i<chain->GetEntries(); ++i){
      if(i%500000==0) std::cout << (i/1000) << "k " << std::endl;
          ...
          for (auto band : error_bands){
            ...
            for (auto universe : error_band_universes){
              ...
              for (auto v : variables2d){
                ...
                // Is it truly in the given target?
                if(targetID<10 && universe->GetInt("truth_targetID") == targetID){
                  v->m_selected_mc_sb.GetComponentHist("True_Target")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                  v->m_selected_mc_sb.GetComponentHist("True_Target_trueval")->Fill(v->GetTrueValue(*universe, 0), universe->GetTruthWeight());
                  if (v->GetName()=="Enu") trueTarget++;
                }

                else if(universe->GetInt("truth_targetID") == 0) {
                  if(universe->GetVecElem("NukeCC_vtx", 2) < universe->GetVecElem("mc_vtx", 2)){ // DS
                    v->m_selected_mc_sb.GetComponentHist("DS")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                  ...
                  v->m_selected_mc_sb.GetComponentHist("US")->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
        ...
        if( planeVal < 1 ) continue; // weird data point at PlaneNumber 1...
  ...
  std::cout << "True Target = " << trueTarget << std::endl;

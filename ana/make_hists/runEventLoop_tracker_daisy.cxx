...
#include "../include/VariableRunTrackerDaisy.h"  
...
int main(int argc, char *argv[]){
  ...
  for (auto v : variablesData) {
    ...
    for(int petal=0; petal<12; petal++){
      v->daisy_petal_data_hists[petal].SyncCVHistos();
    }
  ...
  for (auto v : variables2DData) {
    v->m_selected_data_reco.SyncCVHistos();
    for(int petal=0; petal<12; petal++){
      v->daisy_petal_data2d_hists[petal].SyncCVHistos();
    }
  }
...
void FillVariable( PlotUtils::ChainWrapper* chain, HelicityType::t_HelicityType helicity, NukeCCUtilsNSF *utils , NukeCC_Cuts *cutter ,NukeCC_Binning  *binsDef ,std::vector<Var*>& variables,std::vector<Var2D*>& variables2d,bool isMC,int targetID, int targetZ, const string playlist, bool doDIS){
  ...
  for(int i=0; i<chain->GetEntries(); ++i){
    if(i%5000==0) std::cout << (i/1000) << "k " << std::endl;
              ...
              if( 1 == universe->GetInt("mc_current") &&  -14 == universe->GetInt("mc_incoming") ){
                if(cutter->TrackerOnlyTrue(universe)){
                  if( v->GetName()!="Emu")  if(cutter->PassTrueMuEnergyCut(universe)){
                    ...
                    // background out of muon energy range
                    v->daisy_petal_mc_hists_bkg[petal].univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
      ...
      // What DAISY PETAL the event is in?
      //========================================
      int petal = TargetUtils::Get().GetDaisyPetal(dataverse->GetVertexXMy()*10, dataverse->GetVertexYMy()*10 ); // multiply from cm to mm
      // input must be in mm
      //std::cout << "Petal" << petal << std::endl; 
      ...
      for (auto v : variables2d){
        ...
        v->daisy_petal_data2d_hists[petal].hist->Fill(v->GetRecoValueX(*dataverse), v->GetRecoValueY(*dataverse));
      }
      ...
      for (auto v : variables){
        ...
        v->daisy_petal_data_hists[petal].hist->Fill(v->GetRecoValue(*dataverse, 0));

...
allfilespot = [f for f in os.listdir(path2) if f.endswith('.root')]
#os.listdir(path)
...
_filePOT = ROOT.TFile(path2+"/"+allfilespot[0],"r")
...
# first file pot
mcPOT = _filePOT.Get("MCPOT").GetVal()
dataPOT = _filePOT.Get("DataPOT").GetVal()
print mcPOT
print dataPOT
...
# define outfile
_outfile = ROOT.TFile(path+"/"+str(sys.argv[3]),"RECREATE") # write to the directory of files that were merged
_outfile.cd()
...
for i in range(1,len(allfiles)):
    ...
    _filePOT = ROOT.TFile(path2+"/"+allfilespot[i],"r")

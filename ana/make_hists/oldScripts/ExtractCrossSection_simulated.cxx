...
------"<<std::endl;
  ...
  // output file
  TFile *fUnfold = new TFile( Form("%s/SimCrossSection_t%d_z%d_%s.root", outdir.c_str(), targetID, targetZ, plist_string.c_str() ), "recreate" );
  
  // read in the POT information
  TParameter<double> *mcPOT = (TParameter<double>*)fEfficiency->Get("MCPOT");
  TParameter<double> *dataPOT = (TParameter<double>*)fEfficiency->Get("DataPOT");
  ...
  for(const auto&var: vars){
    ...
    effDenom->Clone()->Write(Form("effDenom_%s", var.c_str() ));


    // CROSS-SECTION EXTRACTION

    // FLUX INFO
    std::cout << "Playlist used in flux reweighter\n" << std::flush;
    std::cout << plist_string << "\n" << std::flush;
    auto& frw = PlotUtils::flux_reweighter(plist_string,-14, true, 100);
    auto fluxIntegral = frw.GetIntegratedFluxReweighted(-14, effDenom, 0, 120, false);


    // TARGET INFO
    PlotUtils::TargetUtils targetInfo;
    double nNucleons = targetInfo.Get().GetTrackerNNucleons(5980, 8422, true, 850);
    //targetInfo.GetPassiveTargetNNucleons( targetID, targetZ, true, 850 );
    std::cout << "Number of nucleons in the target\n" << std::flush;
    std::cout << nNucleons << "\n" << std::flush;

    //targetInfo.Get().GetTrackerNNucleons(5980, 8422, true, 850);
    //const int nplanes = PlotUtils::TargetUtils::Get().GetNPlanes(5980, 8422);
    //double nucleons_check = PlotUtils::TargetUtils::Get().GetTrackerNNucleons( nplanes, true, 850);
    //std::cout << "nNucleons " << nNucleons<<std::endl;
    //std::cout << "PLANES " << nplanes <<std::endl;
   // std::cout << "nNucleons check " << nucleons_check <<std::endl;

    //double TargetUtils::GetTrackerNNucleons( double minZ, double maxZ, bool isMC, double apothem /* = 850. */ ) const
    auto effDenom2 = effDenom->Clone();
    std::cout << "MC POT\n" << endl;
    std::cout << mcpot << "\n" << endl;
    auto simCrossSection = normalize(effDenom2, fluxIntegral, nNucleons, mcpot);
    //Plot(*crossSection_mc, "crossSection_mc", prefix);
    simCrossSection->Clone()->Write(Form("simCrossSection_%s", var.c_str() ));
    std::cout << "Differential cross-section DONE.\n" << std::flush;

...
------"<<std::endl;
 ...
 if(var == "planeDNN"){
  ...
  if( ! tune_fiducial ){
    scaleFactor = new TFile( Form("%s/CHScaleFactors_targetsCombined_t%d_z%02d_%s_%s.root", outdir.c_str(), targetID, targetZ, playlist.c_str(), getenv("NUKECC_TAG") ), "recreate" );
    assert(scaleFactor);
  }
...
MnvH1D*  TunePlasticBackground( MnvH1D *histo, string region, int targetZ, string playlist, string var ){
    ...
    TFile *scaleFactor = new TFile( Form("%s/CHScaleFactors_targetsCombined_z%d_%s_%s.root", getenv("FILES"), targetZ, playlist.c_str(), getenv("NUKECC_TAG") ), "read" );
    assert(scaleFactor);

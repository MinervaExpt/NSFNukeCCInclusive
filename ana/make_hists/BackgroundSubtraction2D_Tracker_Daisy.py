...
for var in vars:
  for petal in range(0,12):
    ...
    reco = infile.Get("selected_mc_reco2d_daisy_%s_%s"%(petal,var))
    bkg = infile.Get("selected_mc_reco2d_bkg_daisy_%s_%s"%(petal,var))
    data = infile.Get("selected_data_reco2d_daisy_%s_%s"%(petal,var))

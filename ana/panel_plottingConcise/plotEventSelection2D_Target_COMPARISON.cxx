...
void makePlots(bool doMultipliers,bool doRatio, string indir, string outdir, int targetID, int targetZ, string plist, int combined)
{
  ...
  TString histFileName, histFileName2;
  histFileName = Form("%s/minervame5A/EventSelection/EventSelection_minervame5A_t%d_z%02d_sys.root", indir.c_str(), targetID, targetZ);
  histFileName2 = Form("%s/minervame6A/EventSelection/EventSelection_minervame6A_t%d_z%02d_sys.root", indir.c_str(), targetID, targetZ);

 
  TFile *f1 = new TFile( histFileName,"read" );
  TFile *f2 = new TFile( histFileName,"read" );

  MnvH2D* dataMnv=(MnvH2D*)f1->Get("selected_data_reco2d_pZmu_pTmu");
  MnvH2D* mcMnv=(MnvH2D*)f1->Get("selected_mc_reco2d_pZmu_pTmu");
  
  MnvH2D* dataMnv2=(MnvH2D*)f2->Get("selected_data_reco2d_pZmu_pTmu");
  MnvH2D* mcMnv2=(MnvH2D*)f2->Get("selected_mc_reco2d_pZmu_pTmu");

  //MnvH2D* mcMnv_USplastic =(MnvH2D*)f1->Get("selected_mc_reco2d_USplastic_pZmu_pTmu");
  //MnvH2D* mcMnv_DSplastic =(MnvH2D*)f1->Get("selected_mc_reco2d_DSplastic_pZmu_pTmu");
  //MnvH2D* mcMnv_other =(MnvH2D*)f1->Get("selected_mc_reco2d_other_pZmu_pTmu");
  ...
  TParameter<double> *pot_mc1 = (TParameter<double>*)f1->Get("MCPOT");
  TParameter<double> *pot_data1 = (TParameter<double>*)f1->Get("DataPOT");

  TParameter<double> *pot_mc2 = (TParameter<double>*)f2->Get("MCPOT");
  TParameter<double> *pot_data2 = (TParameter<double>*)f2->Get("DataPOT");

  double MCPOT1 = pot_mc1->GetVal();
  double DataPOT1 = pot_data1->GetVal();
  cout << "MC POT ME5A: " << MCPOT1 << endl;
  cout << "Data POT ME5A: " << DataPOT1 << endl;

  double MCPOT2 = pot_mc2->GetVal();
  double DataPOT2 = pot_data2->GetVal();
  cout << "MC POT ME6A: " << MCPOT2 << endl;
  cout << "Data POT ME6A: " << DataPOT2 << endl;

  double scale1 = DataPOT1/MCPOT1;
  double scale2 = DataPOT2/MCPOT2;

  dataMnv->Add(dataMnv2);
  mcMnv->Scale(scale1);
  mcMnv2->Scale(scale2);
  mcMnv->Add(mcMnv2);
  ...
  //TH2* mc_USplastic =new TH2D(mcMnv_USplastic->GetCVHistoWithStatError());
  //TH2* mc_DSplastic =new TH2D(mcMnv_DSplastic->GetCVHistoWithStatError());
  ...
  //H2* mc_NC =new TH2D(mcMnv_NC->GetCVHistoWithStatError());
  ...
  //mc_USplastic->SetLineColor(30);
  //mc_DSplastic->SetLineColor(kOrange-2);
  //mc_other->SetLineColor(38);
  //mc_WrongSign->SetLineColor(kMagenta+2);
  //mc_NC->SetLineColor(kGray);

  //mc_USplastic->SetLineWidth(2);
  //mc_DSplastic->SetLineWidth(2);
  ...
  //histAndOpts.push_back(std::make_pair(mc_USplastic,       "graph0LX"));
  //histAndOpts.push_back(std::make_pair(mc_DSplastic,       "graph0LX"));
...
int main(int argc, char* argv[])
{
  ...
  int targetID = 2;
  int targetZ = 26;
  const string playlist= "minervame5A6A";
  int combined = 0;

...
void makePlots(bool doMultipliers,bool doRatio, string indir, string outdir, int targetID, int targetZ, string plist, int combined)
{
  ...
  if(targetID==99){
    title->SetHeader(Form("%s: Bkg Subtracted", trueZ.c_str()),"C"); // option "C" allows to center the header
  }
   else {
  ...
  if(doRatio){
    ...
    gc->Print(doMultipliers ? Form("%s/BkgSubtracted_EventSelection2D_t%d_z%02d_%s_pt_multiplier.png", outdir.c_str(), targetID, targetZ, plist.c_str()) : Form("%s/BkgSubtracted_EventSelection2D_t%d_z%02d_%s_pt.png", outdir.c_str(), targetID, targetZ, plist.c_str()));
  ...
  if(targetID==99){
    title2->SetHeader(Form("%s: Bkg Subtracted", trueZ.c_str()),"C"); // option "C" allows to center the header
  }
   else {
  ...
  if(doRatio){
    ...
    gc2->Print(doMultipliers ? Form("%s/BkgSubtracted_EventSelection2D_t%d_z%02d_%s_pz_multiplier.png", outdir.c_str(), targetID, targetZ, plist.c_str()) : Form("%s/BkgSubtracted_EventSelection2D_t%d_z%02d_%s_pz.png", outdir.c_str(),  targetID, targetZ, plist.c_str()));

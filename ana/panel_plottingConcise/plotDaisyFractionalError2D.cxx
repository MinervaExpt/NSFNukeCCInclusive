...
// =====================================================================
void makePlots(int petal, bool pt, bool drawGroups, int areanorm,  string indir, string outdir, int targetID, int targetZ, string plist)
{
  ...
  histFileName = Form("%s/EventSelection_daisy_%s_t%d_z%02d_sys.root", indir.c_str(), plist.c_str(), targetID, targetZ);
 
  TFile *f = new TFile( histFileName,"read" );

  MnvH2D* data=(MnvH2D*)f->Get(Form("selected_mc_reco2d_daisy_%d_pZmu_pTmu",petal));
  ...
  if(!drawGroups){
    ...
    if(pt){
      ...
      // Adding title!
      if(targetID==99){
        title->SetHeader(Form("Daisy %s: Petal %d", trueZ.c_str(), petal),"C"); // option "C" allows to center the header
      }
      else {
      ...
      gcPT->Print(Form("%s/EventSelection2D_FracErr_t%d_z%02d_%s_pt_p%d_%d.png", outdir.c_str(), targetID, targetZ, plist.c_str(),petal,areanorm));
      ...
      if(targetID==99){
        title2->SetHeader(Form("Daisy %s: Petal %d", trueZ.c_str(), petal),"C"); // option "C" allows to center the header
      }
      else {
      ...
      gcPT->Print(Form("%s/EventSelection2D_FracErr_t%d_z%02d_%s_pz_p%d_%d.png", outdir.c_str(), targetID, targetZ, plist.c_str(),petal, areanorm));
    ...
    for(int n=0;n<names.size();n++){
      ...
      if(pt){
        ...
        if(targetID==99){
          title3->SetHeader(Form("Daisy %s: Petal %d", trueZ.c_str(), petal),"C"); // option "C" allows to center the header
        }
        else {
          title3->SetHeader(Form("Target %d %s", targetID, trueZ.c_str()),"C"); 
        };
        ...
        gcPT->Print(Form("%s/EventSelection2D_FracErr_t%d_z%02d_%s_pt_p%_%d.png", outdir.c_str(), targetID, targetZ, plist.c_str(),petal,areanorm));
        ...
        if(targetID==99){
          title4->SetHeader(Form("Daisy %s: Petal", trueZ.c_str(), petal),"C"); // option "C" allows to center the header
        }
        else {
          title4->SetHeader(Form("Target %d %s", targetID, trueZ.c_str()),"C"); 
        };
        ...
        gcPT->Print(Form("%s/EventSelection2D_FracErr_t%d_z%02d_%s_pz_p%_%d.png", outdir.c_str(), targetID, targetZ, plist.c_str(),petal,areanorm));
...
int main(int argc, char* argv[])
{
  ...
  for(int petal=0; petal<12; petal++){
    makePlots(petal, true,false, 0, indir, outdir, targetID, targetZ, plist); // not area normalized
  }

...
void makePlots(int petal, bool doMultipliers,bool doRatio, string indir, string outdir, int targetID, int targetZ, string plist, int combined)
{
  ...
  histFileName = Form("%s/BkgSubtracted_EventSelection2D_daisy_%s_t%d_z%02d_sys.root", indir.c_str(), plist.c_str(), targetID, targetZ);
 
  TFile *f1 = new TFile( histFileName,"read" );

  MnvH2D* dataMnv=(MnvH2D*)f1->Get(Form("h_bkg_subtracted_data_daisy_%d_pZmu_pTmu",petal ));
  MnvH2D* mcMnv=(MnvH2D*)f1->Get(Form("h_bkg_subtracted_mc_daisy_%d_pZmu_pTmu", petal));
  ...
  if(targetID==99){
    title->SetHeader(Form("Daisy %s: Petal %d: Bkg Subtracted", trueZ.c_str(), petal),"C"); // option "C" allows to center the header
  }
   else {
    title->SetHeader(Form("Target %d %s: Bkg Subtracted", targetID, trueZ.c_str()),"C"); 
  };
  ...
  if(doRatio){
    ...
    gc->Print(doMultipliers ? Form("%s/BkgSubtracted_EventSelection2D_daisy_t%d_z%02d_%s_pt_p%d_multiplier.png", outdir.c_str(), targetID, targetZ, plist.c_str(), petal) : Form("%s/BkgSubtracted_EventSelection2D_daisy_t%d_z%02d_%s_pt_p%d.png", outdir.c_str(), targetID, targetZ, plist.c_str(), petal));
  ...
  if(doRatio) gc2->SetYTitle("Ratio data/MINERvA Tune v1");
    ...
    if (combined == 1){
      ...
      if(targetID==99){
        gc2->SetYTitle("Events (x10^{3}) per (GeV/c)^{2}");
      }
      else{
  ...
  if(targetID==99){
    title2->SetHeader(Form("Daisy %s: Petal %d: Bkg Subtracted", trueZ.c_str(), petal),"C"); // option "C" allows to center the header
  }
   else {
    title2->SetHeader(Form("Target %d %s: Bkg Subtracted", targetID, trueZ.c_str()),"C"); 
  };
  ...
  if(doRatio){
    ...
    gc2->Print(doMultipliers ? Form("%s/BkgSubtracted_EventSelection2D_daisy_t%d_z%02d_%s_pz_p%d_multiplier.png", outdir.c_str(), targetID, targetZ, plist.c_str(), petal) : Form("%s/BkgSubtracted_EventSelection2D_daisy_t%d_z%02d_%s_pz_p%d.png", outdir.c_str(),  targetID, targetZ, plist.c_str(), petal));

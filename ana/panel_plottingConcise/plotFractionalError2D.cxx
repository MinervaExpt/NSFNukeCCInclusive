...
void makePlots(bool pt, bool drawGroups, int areanorm,  string indir, string outdir, int targetID, int targetZ, string plist)
{
  ...
  if(!drawGroups){
    ...
    if(pt){
      ...
      gcPT->Print(Form("%s/EventSelection2D_FracErr_t%d_z%02d_%s_pt_%d.png", outdir.c_str(), targetID, targetZ, plist.c_str(),areanorm));
      ...
      gcPT->Print(Form("%s/EventSelection2D_FracErr_t%d_z%02d_%s_pz_%d.png", outdir.c_str(), targetID, targetZ, plist.c_str(),areanorm));
    ...
    for(int n=0;n<names.size();n++){
      ...
      if(pt){
        ...
        if(targetID==99){
          title3->SetHeader(Form("%s", trueZ.c_str()),"C"); // option "C" allows to center the header
        }
        else {
        ...
        if(targetID==99){
          title4->SetHeader(Form("%s", trueZ.c_str()),"C"); // option "C" allows to center the header
        }
        else {

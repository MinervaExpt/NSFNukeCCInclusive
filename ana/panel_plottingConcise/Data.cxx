...
//  TFile f1("Hists_EventSelection.root");
  ...
  TFile f2("XSec.root");
  TFile f1("XSecMC.root");
  MnvH2D* dataMnv=(MnvH2D*)f2.Get("CrossSection_pZmu_pTmu");
  MnvH2D* mcMnv=(MnvH2D*)f1.Get("CrossSectionMC_pZmu_pTmu");
  ...
  MnvH2D* mcMnv_qe = (MnvH2D*)f1.Get("CrossSectionMC_QE_pZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_res = (MnvH2D*)f1.Get("CrossSectionMC_resonant_pZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_dis_dis = (MnvH2D*)f1.Get("CrossSectionMC_trueDIS_pZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_dis_sis = (MnvH2D*)f1.Get("CrossSectionMC_softDIS_pZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_2p2h = (MnvH2D*)f1.Get("CrossSectionMC_2p2h_pZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_oth = (MnvH2D*)f1.Get("CrossSectionMC_other_pZmu_pTmu");//Get from N track
...
//  double MCPOT = 9.9935e+18; //one run me6A
//  double DataPOT = 1.23613e+18; //one run me6A

//  double MCPOT = 1.26e+21;
//  double DataPOT = 3.03e+20;  
...
//  double scale = 1.0;
  ...
  mcMnv->Scale(scale*1e39, "width");

//  dataMnv->Scale(1e-5, "width");
//  mcMnv->Scale(scale*1e-5, "width");

  mcMnv_qe->Scale(scale*1e39,"width");
  mcMnv_res->Scale(scale*1e39,"width");
  mcMnv_dis_dis->Scale(scale*1e39,"width");
  mcMnv_dis_sis->Scale(scale*1e39,"width");
  mcMnv_2p2h->Scale(scale*1e39,"width");
  mcMnv_oth->Scale(scale*1e39,"width");

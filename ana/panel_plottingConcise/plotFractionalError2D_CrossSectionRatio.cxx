...
void makePlots(bool pt, bool drawGroups, int areanorm,  string indir, string outdir, int targetID, int targetZ, string plist)
{
  ...
  MnvH2D* xsec_data = (MnvH2D*)f->Get("crossSection_data_pZmu_pTmu");
  MnvH2D* xsec_data_tracker = (MnvH2D*)fdaisy->Get(Form("crossSection_%s_data_pZmu_pTmu", name.c_str()));
  std::cout<< Form("crossSection_%s_data_pZmu_pTmu", name.c_str()) << std::endl;

  MnvH2D* data = xsec_data->Clone();
  data->Divide(xsec_data, xsec_data_tracker);
  ...
  if(!drawGroups){
    ...
    if(pt){
      ...
      // Adding title!
      if(targetID==99){
        title->SetHeader(Form("%s: CrossSection Ratio (Data)", trueZ.c_str()),"C"); // option "C" allows to center the header
      }
      else {
        title->SetHeader(Form("Target %d %s: CrossSection Ratio (Data)", targetID, trueZ.c_str()),"C"); 
      };
      ...
      gcPT->Print(Form("%s/CrossSectionRatio2D_FracErr_t%d_z%02d_%s_pt_%d_Data.png", outdir.c_str(), targetID, targetZ, plist.c_str(),areanorm));
      ...
      if(targetID==99){
        title2->SetHeader(Form("%s: CrossSectio Ratio (Data)", trueZ.c_str()),"C"); // option "C" allows to center the header
      }
      else {
        title2->SetHeader(Form("Target %d %s: CrossSection Ratio (Data)", targetID, trueZ.c_str()),"C"); 
      };
      ...
      gcPT->Print(Form("%s/CrossSectionRatio2D_FracErr_t%d_z%02d_%s_pz_%d_Data.png", outdir.c_str(), targetID, targetZ, plist.c_str(),areanorm));
    ...
    for(int n=0;n<names.size();n++){
      ...
      if(pt){
        ...
        if(targetID==99){
          title3->SetHeader(Form("%s: CrossSection Ratio (Data)", trueZ.c_str()),"C"); // option "C" allows to center the header
        }
        else {
          title3->SetHeader(Form("Target %d %s: CrossSection Ratio (Data)", targetID, trueZ.c_str()),"C"); 
        };
        ...
        if(targetID==99){
          title4->SetHeader(Form("%s: CrossSection Ratio (Data)", trueZ.c_str()),"C"); // option "C" allows to center the header
        }
        else {
          title4->SetHeader(Form("Target %d %s: CrossSection Ratio (Data)", targetID, trueZ.c_str()),"C"); 
        };

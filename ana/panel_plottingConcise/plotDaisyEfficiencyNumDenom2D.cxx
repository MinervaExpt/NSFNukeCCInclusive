...
void makePlots(int petal, bool doMultipliers,bool doRatio, string indir, string outdir, int targetID, int targetZ, string plist)
{
  ...
  histFileName = Form("%s/Efficiency_Daisy_%s_t%d_z%02d_sys.root", indir.c_str(), plist.c_str(), targetID, targetZ);
 
  TFile *f1 = new TFile( histFileName,"read" );

  MnvH2D* numerator=(MnvH2D*)f1->Get(Form("h_mc2d_%d_pZmu_pTmu", petal));
  MnvH2D* denominator=(MnvH2D*)f1->Get(Form("h_truth2d_%d_pZmu_pTmu", petal));
  ...
  if(doRatio){
    ...
    gc->Print(doMultipliers ? Form("%s/Efficiency2D_t%d_z%02d_%s_pt_p%d_multiplier_NumDenom.png", outdir.c_str(), targetID, targetZ, plist.c_str(), petal) : Form("%s/Efficiency2D_t%d_z%02d_%s_pt_p%d_NumDenom.png", outdir.c_str(), targetID, targetZ, plist.c_str(), petal));
  ...
  if(doRatio){
    ...
    gc2->Print(doMultipliers ? Form("%s/Efficiency2D_t%d_z%02d_%s_pz_p%d_multiplier_NumDenom.png", outdir.c_str(), targetID, targetZ, plist.c_str(), petal) : Form("%s/Efficiency2D_t%d_z%02d_%s_pz_p%d_NumDenom.png", outdir.c_str(),  targetID, targetZ, plist.c_str(), petal));

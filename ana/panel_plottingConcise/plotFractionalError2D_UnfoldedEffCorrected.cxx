...
// =====================================================================
void makePlots(bool pt, bool drawGroups, int areanorm,  string indir, string outdir,int filetargetID, int targetID, int targetZ, string plist)
{
  ...
  TString histFileName;
   if (targetID == 99){
    histFileName = Form("%s/CrossSection2D_t%d_z%02d_%s.root", indir.c_str(), filetargetID, targetZ, plist.c_str());
  }
  else{
    histFileName = Form("%s/CrossSection2D_Daisy_t%d_z%02d_%s.root", indir.c_str(), filetargetID, targetZ, plist.c_str());
  }
 
  TFile *f = new TFile( histFileName,"read" );

  MnvH2D* data=(MnvH2D*)f->Get(Form("unfolded_effCorrected_%d_data_pZmu_pTmu",targetID));
  ...
  if(!drawGroups){
    ...
    if(pt){
      ...
      // Adding title!
      if(targetID==99){
        title->SetHeader(Form("%s: UnfoldedEffCorrected (Data)", trueZ.c_str()),"C"); // option "C" allows to center the header
      }
      else {
        title->SetHeader(Form("Target %d %s: UnfoldedEffCorrected (Data)", targetID, trueZ.c_str()),"C"); 
      };
      ...
      gcPT->Print(Form("%s/UnfoldedEffCorrected2D_FracErr_t%d_z%02d_%s_pt_%d_Data.png", outdir.c_str(), targetID, targetZ, plist.c_str(),areanorm));
      ...
      if(targetID==99){
        title2->SetHeader(Form("%s:UnfoldedEffCorrected (Data)", trueZ.c_str()),"C"); // option "C" allows to center the header
      }
      else {
        title2->SetHeader(Form("Target %d %s: UnfoldedEffCorrected (Data)", targetID, trueZ.c_str()),"C"); 
      };
      ...
      gcPT->Print(Form("%s/UnfoldedEffCorrected2D_FracErr_t%d_z%02d_%s_pz_%d_Data.png", outdir.c_str(), targetID, targetZ, plist.c_str(),areanorm));
    ...
    for(int n=0;n<names.size();n++){
      ...
      if(pt){
        ...
        if(targetID==99){
          title3->SetHeader(Form("%s: UnfoldedEffCorrected (Data)", trueZ.c_str()),"C"); // option "C" allows to center the header
        }
        else {
          title3->SetHeader(Form("Target %d %s: UnfoldedEffCorrected (Data)", targetID, trueZ.c_str()),"C"); 
        };
        ...
        if(targetID==99){
          title4->SetHeader(Form("%s: UnfoldedEffCorrected (Data)", trueZ.c_str()),"C"); // option "C" allows to center the header
        }
        else {
          title4->SetHeader(Form("Target %d %s: UnfoldedEffCorrected (Data)", targetID, trueZ.c_str()),"C"); 
        };
...
int main(int argc, char* argv[])
{
  ...
  int filetargetID = atoi(argv[3]);
  int targetID = atoi(argv[4]);
  int targetZ = atoi(argv[5]);
  const string playlist= argv[6];

  const std::string plist(playlist);


  //for(int i=0;i<2;i++){
  makePlots(true,false, 0, indir, outdir,filetargetID, targetID, targetZ, plist); // not area normalized

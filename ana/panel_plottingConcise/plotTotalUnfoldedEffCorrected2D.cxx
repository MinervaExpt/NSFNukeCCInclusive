...
void makePlots(bool doMultipliers,bool doRatio, string indir, string outdir, int targetID, int targetZ, string plist)
{
  ...
  MnvH2D* dataMnv=(MnvH2D*)f1->Get("total_unfolded_effCorrected_data_pZmu_pTmu");
  MnvH2D* mcMnv=(MnvH2D*)f1->Get("total_unfolded_effCorrected_mc_pZmu_pTmu");
  string trueZ;
  string mat;
  ...
  if(targetID==99){
    title->SetHeader(Form("%s: Total UnfoldedEffCorrected", trueZ.c_str()),"C"); // option "C" allows to center the header
  }
   else {
    title->SetHeader(Form("Target %d %s: Total UnfoldedEffCorrected", targetID, trueZ.c_str()),"C"); 
  };
  ...
  if(doRatio){
    ...
    gc->Print(doMultipliers ? Form("%s/TotalUnfoldedEffCorrected2D_t%d_z%02d_%s_pt_multiplier.png", outdir.c_str(), targetID, targetZ, plist.c_str()) : Form("%s/TotalUnfoldedEffCorrected2D_t%d_z%02d_%s_pt.png", outdir.c_str(), targetID, targetZ, plist.c_str()));
  ...
  if(targetID==99){
    title2->SetHeader(Form("%s: Total UnfoldedEffCorrected", trueZ.c_str()),"C"); // option "C" allows to center the header
  }
   else {
    title2->SetHeader(Form("Target %d %s: Total UnfoldedEffCorrected", targetID, trueZ.c_str()),"C"); 
  };
  ...
  if(doRatio){
    ...
    gc2->Print(doMultipliers ? Form("%s/TotalUnfoldedEffCorrected2D_t%d_z%02d_%s_pz_multiplier.png", outdir.c_str(), targetID, targetZ, plist.c_str()) : Form("%s/TotalUnfoldedEffCorrected2D_t%d_z%02d_%s_pz.png", outdir.c_str(),  targetID, targetZ, plist.c_str()));

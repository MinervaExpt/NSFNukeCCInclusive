...
void makePlots(bool doMultipliers,bool doRatio, string indir, string outdir, string material, string plist)
{
  myPlotStyle();
  ...
  title->SetHeader(Form("Cross-Section Ratio: %s", material.c_str()), "C"); 
  ...
  if(doRatio){
    ...
    gc->Print(doMultipliers ? Form("%s/CrossSectionRatio2D_Daisy_%s_%s_pt_multiplier.png", outdir.c_str(), material.c_str(), plist.c_str()) : Form("%s/CrossSectionRatio2D_Daisy_%s_%s_pt.png", outdir.c_str(), material.c_str(), plist.c_str()));
...
int main(int argc, char* argv[])
{
  ...
  const string material = argv[3];
  const string playlist= argv[4];

  const std::string plist(playlist);

  makePlots(false,false, indir, outdir, material, plist);

...
//  ROOT::Cintex::Cintex::Enable();
  ...
  TFile f1("Hists_SidebandsSecondTime.root");
  ...
  MnvH2D* mcMnv_qe = (MnvH2D*)f1.Get("reco_QEpZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_res = (MnvH2D*)f1.Get("reco_resonantpZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_dis_dis = (MnvH2D*)f1.Get("reco_2p2hpZmu_pTmu");//Get from N track
//  MnvH2D* mcMnv_dis_sis = (MnvH2D*)f1.Get("bkg_softDISpZmu_pTmu");//Get from N track
//  MnvH2D* mcMnv_2p2h = (MnvH2D*)f1.Get("bkg_2p2hpZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_oth = (MnvH2D*)f1.Get("reco_otherpZmu_pTmu");//Get from N track

  dataMnv->GetXaxis()->SetTitle("p_{||} (GeV/c)");

  double MCPOT = 6.39843e+20;
  double DataPOT = 1.63037e+20;
...
//  histAndOpts.push_back(std::make_pair(mc_dis_sis,       "graph0LX"));
  ...
  if(doRatio) gc->SetYTitle("Ratio bkgType/Total Background");
  ...
  leg->AddEntry(mc, "Total MC Reconstructed Events", "l");
  leg->AddEntry(mc_qe,"Anti-Neutrino CC","l");
  leg->AddEntry(mc_res,"Neutrino CC","l");
  leg->AddEntry(mc_dis_dis,"Anti-Neutrino NC","l");
//  leg->AddEntry(mc_dis_sis,"Soft DIS","l");
  leg->AddEntry(mc_oth,"Neutrino NC","l");
...
//  GridCanvas* gc2=plotYAxis1D(histAndOpts, "Muon Transverse Momentum (GeV/c)", "p_{||}",4,4,800,500,doMultipliers ? &multipliers2[0] : NULL);
  ...
  if(doRatio) gc2->SetYTitle("Ratio bkgType/Total Background");

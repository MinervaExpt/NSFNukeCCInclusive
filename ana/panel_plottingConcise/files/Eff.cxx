...
//  ROOT::Cintex::Cintex::Enable();
  ...
  TFile f1("Hists_EventSelection_EventInteraction.root");
  TFile f2("Hists_Efficiency.root");
  ...
  MnvH2D* mcMnv_qe = (MnvH2D*)f2.Get("h_mc_pZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_res = (MnvH2D*)f1.Get("reco_resonantpZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_dis_dis = (MnvH2D*)f2.Get("h_truth_pZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_dis_sis = (MnvH2D*)f1.Get("reco_softDISpZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_2p2h = (MnvH2D*)f1.Get("reco_2p2hpZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_oth = (MnvH2D*)f2.Get("h_mc_pZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_bkg = (MnvH2D*)f1.Get("bkg_totalpZmu_pTmu");
...
//  TH2* mc_dis = new TH2D(mcMnv_dis->GetCVHistoWithStatError());
  ...
  mc_oth->Divide(mc_qe,mc_dis_dis, 1.0, 1.0, "B");
...
//  histAndOpts.push_back(std::make_pair(mc_res,       "graph0LX"));
  histAndOpts.push_back(std::make_pair(mc_dis_dis,       "hist"));
//  histAndOpts.push_back(std::make_pair(mc_dis_sis,       "graph0LX"));
//  histAndOpts.push_back(std::make_pair(mc_oth,       "hist"));
...
//  leg->AddEntry(mc, "MINERvA Tune v1", "l");
  leg->AddEntry(mc_qe,"Numerator","l");
//  leg->AddEntry(mc_res,"Resonant","l");
  leg->AddEntry(mc_dis_dis,"Denominator","l");
//  leg->AddEntry(mc_dis_sis,"Soft DIS","l");
//  leg->AddEntry(mc_oth,"Ratio","l");
...
//  leg2->AddEntry(mc, "MINERvA Tune v1", "l");
  ...
  leg3->AddEntry(mc_qe,"Efficiency Numerator","l");
//  leg3->AddEntry(mc_res,"Resonant","l");
  leg3->AddEntry(mc_dis_dis,"Efficiency Denominator","l");

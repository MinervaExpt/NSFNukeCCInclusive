//Sideband breakdown by ANCC, ANNC, NCC, NNC, ANNCCCother - close to becoming F script - Aug27/21
...
//  ROOT::Cintex::Cintex::Enable();
  ...
  TFile f1("Hists_SidebandStudy.root");
  MnvH2D* dataMnv=(MnvH2D*)f1.Get("h_data_pZmu_pTmu");
  MnvH2D* mcMnv=(MnvH2D*)f1.Get("h_mc_pZmu_pTmu");
  ...
  MnvH2D* mcMnv_qe = (MnvH2D*)f1.Get("AntiNeutrino_CC_pZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_res = (MnvH2D*)f1.Get("AntiNeutrino_NC_pZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_dis_dis = (MnvH2D*)f1.Get("Neutrino_CC_pZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_dis_sis = (MnvH2D*)f1.Get("Neutrino_NC_pZmu_pTmu");//Get from N track
  MnvH2D* mcMnv_oth = (MnvH2D*)f1.Get("ANNCCNCother_pZmu_pTmu");//Get from N track
...
double MCPOT = 6.39842697849781485568e+20;
double DataPOT = 1.63036701261034717184e+20;
...
//mc->Scale(scale*1e-5, "width");
//mcTotalError->Scale(scale*1e-5, "width");
...
//  mcTotalError->SetFillColor(kPink +1);
  mc->SetLineWidth(2);
...
//  mc_qe->Add(mc_2p2h);
  ...
  if(doRatio){
    ...
    //    tmpden->Sumw2(false);
//    data->Divide(mcTotalError);
    dataStat->Divide(mcTotalError);
    mc_qe->Divide(mcTotalError);
    mc_res->Divide(mcTotalError);
    mc_dis_dis->Divide(mcTotalError);
    mc_dis_sis->Divide(mcTotalError);
    mc_oth->Divide(mcTotalError);
    mc->Divide(mc);
  }
  ...
  histAndOpts.push_back(std::make_pair(mc,       "HISTL"));
  ...
  if(doRatio) gc->SetYTitle("Ratio data/MC");
  ...
  leg->AddEntry(mc, "Total MC Reco Events", "l");
  leg->AddEntry(mc_qe,"Nu_mubar CC","l");
  leg->AddEntry(mc_res,"Nu_mubar NC","l");
  leg->AddEntry(mc_dis_dis,"Nu_mu CC","l");
  leg->AddEntry(mc_dis_sis,"Nu_mu NC","l");
  leg->AddEntry(mc_oth,"Other","l");
...
//  leg2->AddEntry(mc, "Total Background", "l");
...
//  GridCanvas* gc2=plotYAxis1D(histAndOpts, "Muon Transverse Momentum (GeV/c)", "p_{||}",4,4,800,500,doMultipliers ? &multipliers2[0] : NULL);
  ...
  if(doRatio) gc2->SetYTitle("Ratio data/MC");

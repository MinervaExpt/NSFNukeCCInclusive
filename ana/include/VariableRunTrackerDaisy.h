...
class Variable2D : public PlotUtils::Variable2DBase<NUKECC_ANA::CVUniverse> {
 ...
 public:
  ...
  void InitializeAllHistograms(T univs) {
    ...
    for(int petal=0; petal<12; petal++){
      MH2D* dummy_selected_mc_reco_daisy = new MH2D(Form("selected_mc2d_reco_daisy_%d_%s", petal, name), name, GetNBinsX(),
                 GetBinVecX().data(), GetNBinsY(), GetBinVecY().data());
      daisy_petal_mc2d_hists[petal] = HW2D(dummy_selected_mc_reco_daisy, univs, clear_bands);

      MH2D* dummy_selected_data_reco_daisy = new MH2D(Form("selected_data2d_reco_daisy_%d_%s", petal, name), name, GetNBinsX(),
                 GetBinVecX().data(), GetNBinsY(), GetBinVecY().data());

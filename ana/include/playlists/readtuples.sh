...
do 
    echo $playlist
    # MC
    find "${PWD}" /pnfs/minerva/persistent/DataPreservation/p3/FullDetector/Merged_mc_ana_me6A_DualVertex_p3/ -name 'MasterAnaDev_mc_AnaTuple_run*.root' > ${playlist}_mc_DualVertex_FullDetector_p3.txt
    xrdify ${playlist}_mc_DualVertex_FullDetector_p3.txt > MasterAnaDev_MC_${playlist}.txt
    # Data
    find "${PWD}" /pnfs/minerva/persistent/DataPreservation/p3/Data/Merged_data_ana_me6A_DualVertex_p3/ -name 'MasterAnaDev_data_AnaTuple_run*.root' > ${playlist}_data_DualVertex_p3.txt

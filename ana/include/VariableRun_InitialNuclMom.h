...
class Variable : public PlotUtils::VariableBase<NUKECC_ANA::CVUniverse> {
 ...
 // PlotUtils::MnvH1D* data = new PlotUtils::MnvH1D(
  ...
  //  m_selected_data_sb = PlotUtils::HistFolio<PlotUtils::MnvH1D>(
    ...
    m_selected_mc_sb.AddComponentHist("SRC_neutrons");
    m_selected_mc_sb.AddComponentHist("SRC_protons");
    m_selected_mc_sb.AddComponentHist("SRC");
    m_selected_mc_sb.AddComponentHist("notSRC_protons");
    m_selected_mc_sb.AddComponentHist("notSRC_neutrons");
    m_selected_mc_sb.AddComponentHist("notSRC");
    m_selected_mc_sb.AddComponentHist("Protons");
    m_selected_mc_sb.AddComponentHist("Neutrons");
...
class Variable2D : public PlotUtils::Variable2DBase<NUKECC_ANA::CVUniverse> {
 ...
 public:
  ...
  HW2D m_selected_mc_reco_protons, m_selected_mc_reco_neutrons;
  ...
  void InitializeAllHistograms(T univs) {
    ...
    // HISTWRAPPER
    // selected mc reco histwrapper
    MH2D* dummy_selected_mc_reco_protons =
        new MH2D(Form("mc_reco2d_p_%s", name), name, GetNBinsX(),
                 GetBinVecX().data(), GetNBinsY(), GetBinVecY().data());
    m_selected_mc_reco_protons = HW2D(dummy_selected_mc_reco_protons, univs, clear_bands);

    MH2D* dummy_selected_mc_reco_neutrons =
        new MH2D(Form("mc_reco2d_n_%s", name), name, GetNBinsX(),
                 GetBinVecX().data(), GetNBinsY(), GetBinVecY().data());
    m_selected_mc_reco_neutrons = HW2D(dummy_selected_mc_reco_neutrons, univs, clear_bands);

    MH2D* dummy_selected_mc_reco =
        new MH2D(Form("mc_reco2d_%s", name), name, GetNBinsX(),
                 GetBinVecX().data(), GetNBinsY(), GetBinVecY().data());
    ...
    delete dummy_selected_mc_reco;
    delete dummy_selected_mc_reco_protons;
    delete dummy_selected_mc_reco_neutrons;
    delete dummy_selected_data_reco;
  }
  ...
  void WriteAllHistogramsToFile(TFile& f,bool isMC) const {
    f.cd();
       if(isMC) {m_selected_mc_reco.hist->Write();
        m_selected_mc_reco_protons.hist->Write();
        m_selected_mc_reco_neutrons.hist->Write();
       }
       else m_selected_data_reco.hist->Write();
    // selected mc reco
  }

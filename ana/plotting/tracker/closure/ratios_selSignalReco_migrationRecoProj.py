...
signal = ROOT.TFile("BackgroundSubtracted_EventSelection_bkgSubtract_tracker.root")
mig = ROOT.TFile("Hists_MigrationTracker_ML_ME6A_sys_t99_z99_AntiNu.root")
...
for var in vars:
    ...
    selsig = signal.Get("h_background_subtracted_mc_%s"%var) 
    ...
    ratio.GetYaxis().SetTitle("Reco Signal - Bkg/Mig RecoProj")
    ratio.GetXaxis().CenterTitle()
    ratio.GetYaxis().CenterTitle()
    ratio.Draw("HIST")
    ...
    mnv.AddPlotLabel("Integral Numerator: "+ str(selsig.Integral()), 0.40, 0.87, 0.033, 12, 42)
    mnv.AddPlotLabel("Integral Denominator: "+ str(migraX.Integral()), 0.414, 0.82, 0.033, 12, 42)

    canvas1.Modified()
    canvas1.Update()
    canvas1.Print("ME6A_Closure_selSig_MigRecoProj_ratio_%s.png"%var)

...
bkg = ROOT.TFile("BackgroundSubtracted_EventSelection_bkgSubtract_tracker_TRUE.root")
unfold = ROOT.TFile("CrossSection_t99_z99_minervame6A.root")
...
for var in vars:
    ...
    bkg_sub = bkg.Get("h_background_subtracted_mc_%s"%var) # selected signal
    ...
    ratio.GetYaxis().SetTitle("True Signal - Bkg/Reco Unfolded ")
    ratio.GetXaxis().CenterTitle()
    ratio.GetYaxis().CenterTitle()
    ratio.Draw("HIST")
    ...
    mnv.AddPlotLabel("Integral Numerator: "+ str(bkg_sub.Integral()), 0.40, 0.87, 0.033, 12, 42)

...
for i,g in enumerate(mygraphs):
    ...
    # variable
    print("VAR")
    print(newlist[i].split(".root")[-2]).split("_")[-2]
    var = newlist[i].split(".root")[-2].split("_")[-2]
    petal = newlist[i].split(".root")[-2].split("_")[-1]
    # f factor
    print(float(newlist[i].split(".txt")[-2].split("_")[-1]))
    f = float(newlist[i].split(".txt")[-2].split("_")[-1])
    if var == "Enu":
        mnv.AddHistoTitle("Antineutrino Energy (GeV) %s, f = %f"%(petal,f),  0.04, 1)
        yNDF = 10
        g.SetMaximum(13)
        g.SetMinimum(7)
        #g.SetMaximum(20)
        ...
        ndfLine.Draw("SAME") 
    if var == "x":
        mnv.AddHistoTitle("Bjorken x %s, f = %f"%(petal,f),  0.04, 1)
        yNDF = 6
        g.SetMaximum(9)
        g.SetMinimum(3)
        g.GetXaxis().SetRangeUser(0, 10)
        ...
        ndfLine.Draw("SAME") 
    if var == "pZmu1D":
        mnv.AddHistoTitle("Muon p_{Z} (GeV/c) %s, f = %f"%(petal,f),  0.04, 1)
        yNDF = 13
        g.SetMaximum(16)
        g.SetMinimum(10)
        g.GetXaxis().SetRangeUser(0, 10)
        ...
        ndfLine.Draw("SAME") 
    if var == "pTmu1D":
        mnv.AddHistoTitle("Muon p_{T} (GeV/c) %s, f = %f"%(petal,f),  0.04, 1)
        yNDF = 13
        g.SetMaximum(16)
        g.SetMinimum(10)
        g.GetXaxis().SetRangeUser(0, 10)
        ...
        ndfLine.Draw("SAME") 
    if var == "ThetamuDeg":
        mnv.AddHistoTitle("Muon Angle (Deg) %s, f = %f"%(petal,f),  0.04, 1)
        yNDF = 9
        g.SetMaximum(12)
        g.SetMinimum(6)
        g.GetXaxis().SetRangeUser(0, 10)

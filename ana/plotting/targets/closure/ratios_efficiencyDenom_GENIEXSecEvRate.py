...
efficiency = ROOT.TFile("Hists_Efficiency_ML_ME6A_sys_t3_z26_AntiNu.root")
evRate_file = ROOT.TFile("GENIEXSecExtract_CCInclusive_T3Iron.root")
...
for var in vars:
    ...
    ratio.GetYaxis().SetTitle("Eff. Denom/XSec EvRate")
    ratio.GetXaxis().CenterTitle()
    ratio.GetYaxis().CenterTitle()
    ratio.Draw("HIST")
    ...
    canvas1.Modified()
    canvas1.Update()
    canvas1.Print("ME6A_Closure_EffDenom_GENIEXSecEvRate_ratio_%s2.png"%var)

    ...
    for var in vars:
        ...
        if var == "pTmu1D":
            ...
            if step == "crossSection":
                ...
                mc_hist_C.GetYaxis().SetTitle("Events #times 10^{3} (norm.)")
       
        if var == "pZmu1D":
            mc_hist_C.GetXaxis().SetTitle("Muon p_{Z} (GeV/c)")
            if step == "crossSection":
                mc_hist_C.GetYaxis().SetTitle("d#sigma/dx_{#bar{#nu}} (10^{-39} cm^{2}/x/nucleon)")
                gStyle.SetTitleSize(0.05,"y")
            elif step == "crossSection_total":
        ...
        if var == "ThetamuDeg":
            mc_hist_C.GetXaxis().SetTitle("Muon Angle (Deg)")
            if step == "crossSection":
                mc_hist_C.GetYaxis().SetTitle("d#sigma/dx_{#bar{#nu}} (10^{-39} cm^{2}/x/nucleon)")
                gStyle.SetTitleSize(0.05,"y")
            elif step == "crossSection_total":
        ...
        if var == "x":
            legend = TLegend(0.18,0.75,0.53,0.89)
